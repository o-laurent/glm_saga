...
def toy_example(verbose=False, tol=1e-5):
    ...
    X, y = make_classification(n_features=NUM_FEATURES, random_state=0, n_classes=NUM_CLASSES, n_informative=3)
...
def toy_dataloader_example(verbose=False, tol=1e-5):
    ...
    model = nn.Sequential(nn.Linear(NUM_FEATURES, 10), nn.ReLU(), nn.Linear(10, 5))
    ...
    wloss_sklearn = elastic_loss(
        ...
        ALPHA,
        family="multinomial",
        sample_weight=sample_weight_pth,
    ).item()
    ...
    weighted_loader = add_index_to_dataloader(
        DataLoader(TensorDataset(X, y), batch_size=20, shuffle=True),
        sample_weight=sample_weight_pth,
    )
    train_saga(
        linear,
...
def toy_regression_example(verbose=False, tol=1e-2):
    ...
    X, y = make_regression(n_features=NUM_FEATURES, random_state=0, n_targets=NUM_TARGETS, n_informative=2)
    ...
    dataset = IndexedTensorDataset(X, y)
    loader = DataLoader(dataset, batch_size=20, shuffle=True)
    LAMBDA_loader = maximum_reg_loader(loader, group=False, family="gaussian", preprocess=preprocess) / max(
        0.001, ALPHA
    )
    print("lam check", LAMBDA, LAMBDA_loader / 2)
    assert abs(LAMBDA - LAMBDA_loader / 2) < tol
